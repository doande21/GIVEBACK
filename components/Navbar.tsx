...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-emerald-50 dark:border-emerald-900/30 h-16 flex items-center px-4 transition-colors duration-500">
      <div className="max-w-4xl mx-auto w-full flex items-center justify-between">
        ...
        >
          ...
          <span className="text-xl font-black text-emerald-900 dark:text-emerald-400 tracking-tighter uppercase italic">GIVEBACK</span>
        </div>
        ...
        <div className="flex items-center space-x-1">
          ...
          <button 
            ...
            className="p-2 rounded-xl text-gray-400 hover:text-emerald-500 dark:hover:text-emerald-400 transition-all active:scale-90"
          ...
          >
            ...
            {unreadMessagesCount > 0 && (
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-red-600 text-white text-[7px] font-black flex items-center justify-center rounded-full border-2 border-white animate-bounce">
                {unreadMessagesCount}
              </span>
            )}
          </button>
          ...
          >
            ...
            {pendingRequestsCount > 0 && (
              <span className="absolute top-1 right-1 w-3.5 h-3.5 bg-red-600 text-white text-[7px] font-black flex items-center justify-center rounded-full border-2 border-white">
          ...
          <div 
            className="cursor-pointer ml-1 relative group"
            onClick={() => setActiveTab('profile')}
          >
            <img 
              src={getAvatar(user?.avatar, user?.name)} 
              className="w-8 h-8 rounded-xl border-2 border-emerald-100 dark:border-emerald-800 object-cover bg-gray-100 shadow-sm group-hover:scale-105 transition-transform" 
              alt="Profile"
            />
          </div>
          ...
          <button 
            onClick={() => {
              if (window.confirm("Đệ muốn đăng xuất khỏi hệ thống?")) {
                onLogout();
              }
            }}
