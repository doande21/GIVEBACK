...
const Contact: React.FC = () => {
  const contactMethods = [
    ...
    },
    {
      ...
      link: 'https://facebook.com/giveback'
    }
  ];
  ...
  return (
    <div className="pt-32 pb-20 px-4 max-w-6xl mx-auto min-h-screen">
      <div className="text-center mb-16 relative">
        ...
        <p className="text-emerald-600 dark:text-emerald-400 font-black text-xs uppercase tracking-[0.4em]">Huynh Đệ GIVEBACK luôn sẵn sàng lắng nghe</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {contactMethods.map((method, i) => (
          <a 
            ...
            target="_blank"
            rel="noreferrer"
            className="group relative bg-white dark:bg-slate-900 p-10 rounded-[3.5rem] shadow-xl border border-gray-50 dark:border-slate-800 flex flex-col items-center text-center hover:-translate-y-3 transition-all duration-500 overflow-hidden"
          >
            {/* Background Shape */}
            ...
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 dark:text-gray-500 mb-2 relative">{method.name}</h3>
            <p className="text-xl font-black text-gray-900 dark:text-white mb-10 relative tracking-tight">{method.value}</p>

            <div className={`mt-auto w-full ${method.color} text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-gray-200 dark:shadow-none group-hover:shadow-xl group-hover:scale-[1.02] active:scale-95 transition-all`}>
              {method.label}
            </div>
          </a>
        ))}
