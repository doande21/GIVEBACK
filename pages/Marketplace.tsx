    ...
    <div className="pt-24 pb-12 px-4 max-w-7xl mx-auto">
      ...
      <div className="mb-10 bg-gradient-to-r from-amber-900 via-amber-800 to-amber-900 rounded-[3rem] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
         ...
         <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="max-w-xl text-center md:text-left">
               <h2 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter mb-4">Bạn có món đồ giá trị cao?</h2>
               <p className="text-amber-200 font-bold text-sm md:text-base italic leading-relaxed">
                  Thay vì tặng miễn phí, hãy cùng GIVEBACK tổ chức <span className="text-white underline decoration-amber-400 decoration-2">Đấu giá Gây quỹ</span>. 
                  Toàn bộ số tiền sẽ được dùng để chi trả phí vận chuyển và mua nhu yếu phẩm cho các chuyến cứu trợ vùng xa.
               </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-4">
               <button 
                  onClick={() => setActiveTab?.('contact')} 
                  className="bg-amber-500 hover:bg-amber-400 text-amber-950 px-8 py-4 rounded-2xl font-black uppercase tracking-widest transition-all shadow-xl hover:scale-105 active:scale-95"
               >
                  Liên hệ Admin đấu giá
               </button>
            </div>
         </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-400 font-bold text-[10px] uppercase tracking-widest mt-1">Dành cho các món đồ tặng miễn phí 100%</p>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center px-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-10 animate-in zoom-in-95 duration-200">
             <h2 className="text-2xl font-black uppercase italic text-emerald-900 mb-8">Chi tiết món đồ tặng</h2>
             <div className="space-y-6">
                <input className="w-full bg-gray-50 p-4 rounded-2xl font-bold outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Tên món đồ..." value={newPost.title} onChange={e => setNewPost({...newPost, title: e.target.value})} />
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-4">
                      ...
                      <input placeholder="Địa chỉ nhận" className="w-full bg-gray-50 p-4 rounded-2xl font-bold outline-none" value={newPost.location} onChange={e => setNewPost({...newPost, location: e.target.value})} />
                   </div>
                   <div 
                   ...
                   >
                      {previewMedia ? (
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-[10px] font-black text-emerald-600 uppercase mt-2">Tải lên media</p>
                        </div>
                      )}
                   </div>
                </div>
                <textarea rows={3} placeholder="Mô tả chi tiết..." className="w-full bg-gray-50 p-4 rounded-2xl font-medium outline-none" value={newPost.description} onChange={e => setNewPost({...newPost, description: e.target.value})} />
                <button onClick={handleSubmit} className="w-full bg-emerald-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl">Đăng bài ngay</button>
             </div>
           ...
           <div className="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 duration-300">
              ...
              <div className="p-8">
                 ...
                 <button 
                  ...
                  className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-emerald-100 active:scale-95 transition-all"
                >
                  Liên hệ nhận ngay
                </button>
              </div>
