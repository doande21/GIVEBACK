...
const Notifications: React.FC<NotificationsProps> = ({ user, onNotify, onUpdateUser, onViewProfile }) => {
  ...
  return (
    <div className="pt-24 pb-12 px-4 max-w-2xl mx-auto min-h-screen font-['Inter']">
      <div className="mb-8">
        <h1 className="text-3xl font-black text-Klavika-950  uppercase tracking-tighter leading-none">Trung tâm Thông báo</h1>
        <p className="text-Klavika-600 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Kết nối những tấm lòng đồng điệu</p>
      </div>

      <div className="space-y-4">
        {loading ? (
          <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-8 w-8 border-t-2 border-Klavika-600"></div></div>
        ) : requests.length > 0 ? (
          requests.map(req => (
            <div key={req.id} className="bg-white border border-Klavika-50 p-6 rounded-[2.5rem] flex items-center justify-between shadow-xl animate-in slide-in-from-bottom-2">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p 
                    className="font-black text-sm text-Klavika-950 uppercase  tracking-tighter cursor-pointer hover:text-Klavika-700"
                    onClick={() => onViewProfile(req.fromId)}
                  >
                    {req.fromName}
                  </p>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <button 
                  ...
                  className="bg-Klavika-600 text-white px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-Klavika-100 hover:bg-Klavika-700 active:scale-95 transition-all"
                >
                  Đồng ý
                </button>
                <button 
          ...
          <div className="bg-white rounded-[3rem] p-20 text-center shadow-sm border border-gray-50">
            <div className="w-20 h-20 bg-Klavika-50 rounded-full flex items-center justify-center mx-auto mb-6">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-Klavika-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
