  ...
  return (
    <div className="pt-24 pb-12 px-4 max-w-7xl mx-auto">
      <div className="mb-12 text-center max-w-2xl mx-auto">
        <h1 className="text-5xl font-black text-amber-900 italic uppercase tracking-tighter">Sàn Đấu giá Nhân văn</h1>
      ...
      <div className="mb-16 bg-white border-2 border-dashed border-amber-200 rounded-[3rem] p-8 md:p-12 text-center">
         ...
         <h2 className="text-2xl font-black italic uppercase text-amber-900 mb-4 tracking-tight">Bạn muốn đóng góp vật phẩm đấu giá?</h2>
         <p className="text-gray-500 font-medium text-sm mb-8 max-w-md mx-auto italic leading-relaxed">Nếu bạn sở hữu món đồ có giá trị và muốn đấu giá để gây quỹ, hãy liên hệ trực tiếp với Admin để được thẩm định và lên sàn nhé!</p>
         <button 
          onClick={() => setActiveTab?.('contact')}
          className="bg-amber-600 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-amber-100 hover:bg-amber-700 active:scale-95 transition-all"
        >
          Liên hệ Admin ngay
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {auctions.map(item => (
          <div key={item.id} className="bg-white rounded-[3.5rem] shadow-xl overflow-hidden border border-amber-50 group hover:-translate-y-2 transition-all relative">
            ...
            <div className="p-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-black italic uppercase text-gray-900 truncate pr-2">{item.title}</h3>
              </div>
              
              <div className="bg-amber-50/50 p-6 rounded-[2rem] border border-amber-100 mb-8">
                <div className="flex justify-between items-end">
                   <div>
                      <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest mb-1">Giá hiện tại</p>
                      <p className="text-3xl font-black text-amber-900 tracking-tighter">{item.currentBid.toLocaleString()}đ</p>
                   </div>
                   <div className="text-right">
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Dẫn đầu</p>
                      <p className="text-xs font-bold text-gray-800 truncate max-w-[100px]">{item.highestBidderName || "Chưa có"}</p>
                   </div>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full bg-amber-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-amber-100 hover:bg-black active:scale-95 transition-all"
              >
                Đặt giá thầu
              </button>
            </div>
      ...
      {auctions.length === 0 && (
        <div className="text-center py-20">
           <div className="w-24 h-24 bg-amber-50 text-amber-200 rounded-full flex items-center justify-center mx-auto mb-6">
      ...
      {selectedAuction && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center px-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[4rem] shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 duration-500">
             <div className="h-72 relative">
             ...
             <div className="p-10">
                ...
                <h3 className="text-3xl font-black italic uppercase text-amber-950 mb-4 tracking-tight">{selectedAuction.title}</h3>
                
                <div className="bg-amber-50 p-8 rounded-[2.5rem] border border-amber-100 mb-8 text-center">
                   <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-2">Giá cao nhất hiện tại</p>
                   <p className="text-5xl font-black text-amber-900 tracking-tighter">{selectedAuction.currentBid.toLocaleString()} VNĐ</p>
                   <div className="mt-4 flex items-center justify-center space-x-2">
                      ...
                      <p className="text-[10px] text-amber-800 font-bold italic">{selectedAuction.highestBidderName || "Hãy là người đặt giá đầu tiên"}</p>
                   </div>
                </div>
                ...
                <div className="space-y-6">
                   <div className="relative">
                      <input 
                        type="number" 
                        className="w-full bg-gray-50 border-3 border-transparent focus:border-amber-500 rounded-3xl px-8 py-5 outline-none font-black text-2xl text-amber-900 transition-all"
                   ...
                   <div className="flex justify-between px-2">
                      {[50000, 100000, 500000].map(val => (
                        <button 
                          ...
                          className="bg-amber-50 hover:bg-amber-600 hover:text-white text-amber-600 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                        >
                          +{val/1000}k
                        </button>
                      ))}
                   ...
                   <button 
                    ...
                    disabled={loading}
                    className="w-full bg-amber-950 text-white py-6 rounded-3xl font-black uppercase tracking-[0.2em] shadow-2xl shadow-amber-100 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50"
                  >
                ...
                <div className="mt-10 pt-6 border-t border-gray-100 text-center">
                   ...
                   <p className="text-xs font-black text-amber-900 italic uppercase">{selectedAuction.missionLocation}</p>
                </div>
             </div>
          </div>
        </div>
